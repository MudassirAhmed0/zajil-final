...
const FrieghtDetail = () => {
    ...
    return (
        <div className={`${styles.BlueSection}`}>
            <div className=' xl:px-20 xl:pt-24 px-[18px] pt-[56px] relative'>
                <div className={`flex lg:flex-nowrap flex-wrap-reverse justify-between items-center gap-x-16`}>
                    ...
                    <div ref={imgContainer} className={`img_container img-border rounded-lg overflow-hidden relative ${styles.freightImage}`}>
                        <Image src={warehouse} alt="Global Market" />
                    </div>
                ...
                <div className={`${styles.freightCarousel} flex justify-between mt-28 freightCarousel ${styles.ecoCarousel}`}>
                    <Carousel infinite={false} removeArrowOnDeviceType={['superLargeDesktop', 'desktop',"tablet", "mobile"]} renderArrowsWhenDisabled={false} responsive={responsive} >
                        {content.map(item =>
                            <p onClick={() => setVisibleTab(item.id)} className={`cursor-pointer text-center text-lg text-grey medium-font  w-full	${visibleTab === item.id ? styles.active : styles.noborder}`}>{item.mainHeading}</p>
                        )}
                    </Carousel>
                </div>
            </div>
            <div className={`h-auto ${styles.bottomContent}`}>
                <div className='xl:px-20 xl:py-16 py-[55px] px-[18px]'>
                    {content.map(item =>
                        <div key={item?.id} style={visibleTab === item.id ? {} : { display: 'none' }} className='flex lg:flex-nowrap flex-wrap justify-between gap-x-12'>
                            <span ref={carouselImg} className={`img_container  img-border ${styles.slickImage}`}>
                                <Image src={aeroplane} alt="Aeroplane" />
                            </span>
